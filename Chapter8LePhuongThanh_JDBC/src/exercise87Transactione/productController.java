...
public class productController {
	...
	public List<Product> getAllProduct() throws ClassNotFoundException,
		...
		try (Connection conn = db.connect()) {
			...
			while (resultSet.next()) {
				...
				product.setDate(resultSet.getString("ceateDate"));
	...
	public void transaction1() throws ClassNotFoundException, SQLException,
		...
		try (Connection conn = db.connect()) {
			...
			while (!isId(id)) {
				System.out.println("Enter id againt : ");
			...
			sql = "update product set price = " + product.getPrice() + ", amount = "
					+ product.getAmount() +"where id = " + product.getId()
					+ "";
			statement.equals(sql);
	...
	public void transaction2() throws ClassNotFoundException, SQLException,
			NumberFormatException, IOException, ParseException {
		try (Connection conn = db.connect()) {
			conn.setAutoCommit(false);
			Statement statement = conn.createStatement();
			...
			while(name.length()<150){
				System.out.println("Enter name againt. Name > 150: ");
				name = input.readLine();
			...
			product = new Product(name, price, amount, image, categoryId, date,
					onSell);
			...
			sql = "update product set name = " + product.getName()
					...
					+ product.getAmount() + ", image = " + product.getImage()
					+ " , categoryId = " + product.getCategoryId()
					+ ", createDate = " + product.getDate() + ", onSell = "
					+ product.getOnSell() + " where id = " + product.getId()
					+ "";
			statement.equals(sql);
			...
			while (!isId(idDelete)) {
				System.out.println("Enter id againt: ");
				idDelete = Integer.parseInt(input.readLine());
			}
	...
	public void transaction3() throws ClassNotFoundException, SQLException,
			NumberFormatException, IOException, ParseException {
		try (Connection conn = db.connect()) {
			conn.setAutoCommit(false);
			Statement statement = conn.createStatement();
